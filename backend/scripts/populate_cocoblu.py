...
def run():
    print("Updating Cocoblu Experience...")
    
    new_description = """**Data Pipelines & Platform Architecture**
...
Engineered an **agentic pricing pipeline** utilizing a custom **SP-API SDK** (SigV4 signing, token caching, circuit breakers) that scores product similarity and branches decisioning for automated competitor-based price actioning. Extended the same architectural approach into **Operations Research** -- a hybrid ML/OR system (**ADMM**, **EOQ**) for supply chain inventory optimization."""
    ...
    if existing_exp:
        existing_exp.description = new_description
        existing_exp.technologies = ["AWS Redshift", "AWS Step Functions", "AWS Bedrock", "Claude", "Gemini", "PuLP (MILP)", "FastAPI", "React", "SP-API", "AWS Glue"]
    
    print("Updating Cocoblu Projects...")
    # Project updates
    projects_data = {
        "self-healing-agent": {
    ...
    for p_id, data in projects_data.items():
        existing_p = db.query(Project).filter_by(id=p_id).first()
        if existing_p:
            existing_p.title = data["title"]
            existing_p.description = data["description"]
            existing_p.technologies = data["technologies"]

    db.commit()
    print("Database updated successfully with reorganized domain layout.")
